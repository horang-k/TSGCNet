...
class GraphAttention(nn.Module):
    def __init__(self,feature_dim,out_dim, K):  #feature_dim=12, out_dim=64, k=32
    ...
    def forward(self, Graph_index, x, feature): ## Graph_index = index = knn(coor,k), x = coor, feature = coor1
        ## x = 

        B, C, N = x.shape   #B=batch_size, C=feature_num=64, N=cell_num=16000 (1,64,16000)
        #shpape of feature=coor1 torch.Size([1, 64, 16000, 32])
        #shape of x=coor torch.Size([1, 64, 16000])
        x = x.contiguous().view(B, N, C)    #(1,16000,64)
        feature = feature.permute(0,2,3,1)  #f^ 
        #  torch.Size([1, 16000, 32, 64])
        neighbor_feature = index_points(x, Graph_index) #f_ij
        centre = x.view(B, N, 1, C).expand(B, N, self.K, C) #f_i
        delta_f = torch.cat([centre-neighbor_feature, neighbor_feature], dim=3).permute(0,3,2,1) #delta_f-cat-f_ij
        e = self.conv(delta_f)
        e = e.permute(0,3,2,1)
